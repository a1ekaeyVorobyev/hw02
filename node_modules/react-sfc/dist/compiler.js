...
function Compiler(args) {
            ...
            if (node.type === "ExportDefaultDeclaration") {
                ...
                if (_node.body.type === "BlockStatement") {
                    ...
                    if (RSArg.type === "JSXElement") {
                        exportDefaultNode.pos_HeadOfDefault = _node.body.start + 1;
                ...
                if (!hasExportDefault) {
                    throw new Error("a React SFC must export default a function component. None was detected. Pls file an issue if this is a mistake.");
                }
            }
            // usestate
    ...
    // validations
    if (exportDefaultNode.pos_HeadOfDefault === undefined)
        return;
    /*
    ...
    */
    ...
    if (STYLEDECLARATION && STYLECONTENT) {
        ...
        if (exportDefaultNode.lastChildofDefault)
            ms.appendRight(exportDefaultNode.lastChildofDefault.end, `<style jsx>{${STYLECONTENT}}</style>`);
    }
    // useState
    if (stateMap.size) {
        // for each state hook
...
function use${key}_State(v) {
  ...
  ${isProduction
    ...
    if (options.componentDisplayName && exportDefaultNode.node) {
        const componentDeclarationNode = exportDefaultNode.node.declaration;
        ms.overwrite(exportDefaultNode.node.start, exportDefaultNode.node.end, `const $$Component = ${ms.slice(componentDeclarationNode.start, componentDeclarationNode.end)};
